...
for i in range( len( chi2R_file ) ) :
	...
	v0_x[i]    = float(chi2R_file[i][0:-1].split(' ')[5])
	v0_y[i]    = float(chi2R_file[i][0:-1].split(' ')[6])
	v0_z[i]    = float(chi2R_file[i][0:-1].split(' ')[7])
	v0[i]      = float(chi2R_file[i][0:-1].split(' ')[8])
	the_v[i]   = np.rad2deg(np.arccos(  v0_z[i] /  v0[i] ) )
	phi_v[i]   = np.rad2deg(np.arctan2( v0_y[i], v0_x[i] ) )
	w_per[i]   = float(chi2R_file[i][0:-1].split(' ')[9])
	w_par[i]   = float(chi2R_file[i][0:-1].split(' ')[10])
	w[i]       = float(chi2R_file[i][0:-1].split(' ')[11])
	Rchi2fc[i] = float(chi2R_file[i][0:-1].split(' ')[12])
	Rchi2pl[i] = float(chi2R_file[i][0:-1].split(' ')[13])
...
axs1[0].errorbar( s, n, yerr=n_sig_r, color='k', ls='', elinewidth=2 )
axs1[0].set_ylabel( r'$n_{pc}$ $(cm^{-3})$, $\sigma_n/n$', fontsize=18 )
...
axs1[1].axhline( max(g), c='b', lw='1', label=r'$max(g)$')
axs1[1].axhline( min(g), c='r', lw='1', label=r'$min(g)$' )
axs1[1].scatter( s, g, color='k' )
axs1[1].errorbar( s, g, yerr=g_sig_r, color='k', ls='', elinewidth=2 )
axs1[1].set_ylabel( r'$g$, $\sigma_g/g$', fontsize=18 )
...
axs1[1].set_ylabel( r'${\chi}^{2(R)}_{PL}$', fontsize=18 )
axs1[1].set_xscale( 'log' )
...
axs1[2].set_ylabel( r'${\chi}^{2(R)}_{FC}/{\chi}^{2(R)}_{PL}$', fontsize=18 )
axs1[2].set_xscale( 'log' )
axs1[2].set_yscale( 'log' )
axs1[2].axhline( 1, c='g', lw='1' )
axs1[2].set_xlim( 1e-20, 1e-14 )
axs1[2].set_ylim( 0.01, 100 )
axs1[2].set_xlabel( r's $({\chi}^{2(R)} \textasciitilde 1/(s{\sigma})^2)$', fontsize=18 )
...
plt.savefig( os.path.join( 'chi-squared', date, 'chi-squared_test_plots_chi2' +'.pdf' ), bbox_inches='tight', dpi=40 )
plt.savefig( os.path.join( 'chi-squared', date, 'chi-squared_test_plots_chi2' +'.eps' ), bbox_inches='tight', dpi=40 )
