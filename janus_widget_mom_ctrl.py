...
class widget_mom_ctrl( QWidget ) :
	...
	def __init__( self, core ) :
		...
		self.lab_win_dir_pl = QLabel( 'Directions:'         )
		self.lab_win_bin_pl = QLabel( 'Bins per direction:' )

		font = QFont( "Helvetica", 12 )

		self.lab_win_dir.setFont( font )
		self.lab_win_bin.setFont( font )
		self.lab_win_dir_pl.setFont( font )
		self.lab_win_bin_pl.setFont( font )

		font.setBold(True)

		self.lab_win_fc.setFont( font )
		self.lab_win_pl.setFont( font )

		# Row by row, add the text areas, buttons, and labels to this
		# widget's grid.

		self.grd.addWidget( self.lab_win_fc,  0, 0, 1, 1 )	

		self.grd.addWidget( self.lab_win_dir, 1, 0, 1, 1 )
		self.grd.addWidget( self.txt_win_dir, 1, 1, 1, 1 )

		self.grd.addWidget( self.lab_win_bin, 2, 0, 1, 1 )
		self.grd.addWidget( self.txt_win_bin, 2, 1, 1, 1 )

		self.grd.addWidget( self.lab_win_pl,  3, 0, 1, 1 )

		self.grd.addWidget( self.lab_win_dir_pl, 4, 0, 1, 1 )
		self.grd.addWidget( self.txt_win_dir_pl, 4, 1, 1, 1 )

		self.grd.addWidget( self.lab_win_bin_pl, 5, 0, 1, 1 )
		self.grd.addWidget( self.txt_win_bin_pl, 5, 1, 1, 1 )
